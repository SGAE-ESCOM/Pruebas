...
class Editar_Preguntas_Respuestas:
    ...
    def editar_preguntas_respuestas(self):
        ...
        for i in range(0,len(datos)):
            ...
            time.sleep(1)
            btn_actualizar_modal = self.driver.find_element_by_name('actualizarItem')
            inp[i].clear()
            inp[i].send_keys(datos[i])
            time.sleep(2)
            #Eliminar imagenes 
            
            ei.append(self.driver.find_elements_by_xpath('/html/body/div[2]/div[2]/div/mat-dialog-container/modal-pregunta/div/div/app-form-preguntas/div/div/div[2]/div/form/div/app-lista-opciones/form/div[2]/mat-form-field/div/div[1]/div[1]/ngx-mat-file-input/input'))
            time.sleep(1)
            ei[i].clear()
            time.sleep(2)
            btn_actualizar_modal.click()
            time.sleep(2)
        print("... Test Case 'Editar Preguntas Respuestas' Successful ...")
        print("==========================================================")
